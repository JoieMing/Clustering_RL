...
# data
parser.add_argument('--dataset', type=str, default='amap', help='type of dataset.')
...
# E net
parser.add_argument('--E_epochs', type=int, default=600, help='Number of epochs to train E.')
...
parser.add_argument('--dir', type=str, default = "./logs/1209_amap/",help= "save directory for MLP model.")
...
parser.add_argument('--save_Qfile_name', type=str, default="Q_net.pth", help='save file name for MLP model.')

args = parser.parse_args()
issave = args.issave
save_path = args.dir + args.save_file_name
save_Q_path = args.dir + args.save_Qfile_name
device = "cuda:0"
file_name = "result.csv"
# for args.dataset in ["bat", "eat", "cora", "citeseer"]:
# for args.dataset in ["amap", "cora", "citeseer"]:
for args.dataset in ["amap"]:
    ...
    for epoch in tqdm(range(args.E_epochs)):
        ...
        os.makedirs(os.path.dirname(args.dir), exist_ok=True)
        np.save(args.dir + "reward_list.npy", reward_array)
        # np.save( "reward_list.npy", reward_array)
        # print("Reward list saved to 'reward_list.npy'")
        ...
        if len(replay_buffer) >= args.replay_buffer_size:
            for it in range(args.Q_epochs):
                ...
                # Ensure the directory exists
                os.makedirs(os.path.dirname(save_Q_path), exist_ok=True)
                torch.save(Q_net.state_dict(), save_Q_path)
            # cleaning up
            replay_buffer = []
    ...
    mlp_model = my_model([sm_fea_s.shape[1]] + args.dims).to(device)

    # Load the saved model weights
    mlp_model.load_state_dict(torch.load(save_path))
    # Set the model to evaluation mode
    mlp_model.eval()
    # Load the Q-net model
    Q_net_eval = my_Q_net(args.dims + [256, 9]).to(device)
    Q_net_eval.load_state_dict(torch.load(save_Q_path))
    Q_net_eval.eval()
    for episode in range(n_eval_episodes):
        ...
        # # Set the model to evaluation mode
        # mlp_model.eval()
        z1, z2 = mlp_model(sm_fea_s.to(device))
        embedding = (z1 + z2) / 2
        ...
        action = int(Q_net_eval(state, cluster_state).mean(0).argmax())
        
        # Perform clustering based on the chosen action
        cluster_num = action + 2
        cluster_nums.append(cluster_num)

        nmi, ari, _, _, _ = clustering(embedding.detach(), true_labels,cluster_num, device = device)

        nmi_list_eval.append(nmi)
        ari_list_eval.append(ari)
# Display evaluation results
print("Evaluation Completed.")
print(f"Average NMI: {np.mean(nmi_list_eval):.4f}, Standard Deviation: {np.std(nmi_list_eval):.4f}")
print(f"Average ARI: {np.mean(ari_list_eval):.4f}, Standard Deviation: {np.std(ari_list_eval):.4f}")
print(f"Average Cluster Number: {np.mean(cluster_nums):.4f}, Standard Deviation: {np.std(cluster_nums):.4f}")