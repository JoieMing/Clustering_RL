...
if __name__ == "__main__": 
    ...
    mean_reward, mean_action = evaluate_and_record_average_action(model, env, n_eval_episodes=10)

    print(f"Mean Reward: {mean_reward}")
    print(f"Average Action: {mean_action[0]}")
    print(f"The number of clusters: {np.round(mean_action[0])}")

    
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    nmi, ari = get_embedding(args,device,cluster_num = np.round(mean_action[0]).astype(int))
    print(f"NMI: {nmi}")
    print(f"ARI: {ari}")